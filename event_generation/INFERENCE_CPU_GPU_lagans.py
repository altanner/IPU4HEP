...
loc = Sequential(
    [
        ...
        LeakyReLU(),
        LocallyConnected2D(
            1, (2, 2), use_bias=False, kernel_initializer="glorot_normal"
        ),
        Activation("relu"),
    ]
)
...
for batch_size_i in batch_size_i_array:
    ...
    t0 = time.time()
    generated_training = tf.while_loop(
        condition_func, body, loop_vars=[loop_index, output]
    )[1]
    t1 = time.time()
    total_time = t1 - t0
    ...
    print(
        "Time to generate %d points: %.3fs"
        % (np.shape(generated_training)[0], total_time)
    )

    print(
        "Points generated in 5 mins: %d" % int(points_in_5_mins),
        "batch_size",
        batch_size,
    )
