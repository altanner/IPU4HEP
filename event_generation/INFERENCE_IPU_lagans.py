...
loc = Sequential(
    [
        ...
        Conv2D(
            int(factor * 64), (5, 5), padding="same", kernel_initializer="he_uniform"
        ),
        LeakyReLU(),
        BatchNormalization(),
...
emb = Flatten()(
    Embedding(2, latent_size, input_length=1, embeddings_initializer="glorot_normal")(
        image_class
    )
)
...
with tf.Session() as sess:
    ...
    sess.run(
        run_loop, feed_dict={numPoints: int(loops_to_do / batch_size)}
    )  # 1E5 events for test
    ...
    if batch_size > 1:
        generated_training = np.reshape(
            generated_training,
            (np.shape(generated_training)[0] * np.shape(generated_training)[1], 25, 25),
        )
...
print(
    "Time to generate %d points: %.3fs" % (np.shape(generated_training)[0], total_time)
)

print(
    "Points generated in 1 sec: %d." % int(points_per_second), "batch_size", batch_size
)
