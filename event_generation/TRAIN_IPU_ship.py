...
train_images = train_images.reshape(train_images.shape[0], 1, 4).astype("float32")
...
optimizer_D = tf.compat.v1.train.AdamOptimizer(
...
def train_model(X):
    ...
    return (
        outfeed_FULL.enqueue([loss_D, loss_stacked, fake_images]),
        training_op_D,
        training_op_stacked,
    )
